...
int main(int argc, char *argv[])
{
    ip_addr_t src, dst;
    uint16_t id, seq = 0;
    size_t offset = IP_HDR_SIZE_MIN + ICMP_HDR_SIZE; /* ICMPヘッダは自分で生成するのでデータの先頭を指すようにする */
    ...
    id = getpid() % UINT16_MAX; /* プロセスID(pid)からidを採番する */

    /* Ctrl+Cが押されるとシグナルハンドラon_signal()の中でterminateに1が設定される */
    while (!terminate)
    {
        /* 1秒おきにデバイスにパケットを書き込む */
        if (icmp_output(ICMP_TYPE_ECHO, 0, hton32(id << 16 | ++seq) /* 32bit値をネットワークバイトオーダーにして渡す */, test_data + offset, sizeof(test_data) - offset, src, dst) == -1)
        {
